...
def main():
    ...
    while True:
        try:
            num_movies = int(
                input("How many movies would you like to enter? "))
        except ValueError:
    ...
    if missing_result == False:
        ...
        for id in movie_IDs:
            ...
            print("Movie " + str(len(movie_objects)) +
                  " found: "+ia.get_movie(id).get('title'))
        ...
        for cast in movie_casts:
            if not cast:
                cast_exist = False
                print("Unfortunately there is no cast for " +
                      movie_objects[i].get('title'))
            ...
            if common_actors:
                ...
                for actor in common_actors:
                    ...
                    if len(sys.argv) > 1:
                        if sys.argv[1] == '-wiki':
                            import webbrowser
                            # makes annoying output in CLI, but not much of an issue
                            webbrowser.open(
                                'https://en.wikipedia.org/wiki/'+actor.get('name'))
            else:
